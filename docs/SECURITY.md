...
Камера и микрофон требуют системных разрешений Android.

Основная камера integration v0.3.4.1.4 работает через Home Assistant MJPEG proxy:

```text
Tablet -> Home Assistant -> remote/local client
```

Для основной камеры не требуется и не рекомендуется публиковать наружу порты планшета. В частности, не пробрасывайте в Интернет:

- `2323` — локальный API HA Kiosk Local;
- `8554` — дополнительный RTSP server.

Direct WebRTC и RTSP остаются дополнительными режимами для локальной сети. Монитор движения/звука может работать в foreground service, если соответствующие функции включены.
